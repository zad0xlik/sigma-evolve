...
def get_memory_client(custom_instructions: str = None):
    ...
    try:
        ...
        if _memory_client is None or _config_hash != current_config_hash:
            ...
            # Try to initialize with retries for transient connection issues
            max_retries = 3
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    print(f"Memory client initialized successfully on attempt {attempt + 1}")
                    break
                except Exception as init_error:
                    if attempt < max_retries - 1:
                        print(f"Warning: Failed to initialize memory client (attempt {attempt + 1}/{max_retries}): {init_error}")
                        print(f"Retrying in {retry_delay} seconds...")
                        import time
                        time.sleep(retry_delay)
                    else:
                        print(f"Error: Failed to initialize memory client after {max_retries} attempts: {init_error}")
