...
from openmemory.app.models import KnowledgeExchange
...
from openmemory.app.utils.knowledge_exchange import KnowledgeExchangeProtocol
...
def test_duplicate_knowledge_scenarios():
    ...
    # Scenario 2: Near-duplicate with slight variations
    print("\n2. Near-duplicate with slight variations...")
...
async def test_real_world_scenario():
    ...
    try:
        # Simulate a real workflow where workers share knowledge
        print("\n1. Setting up workers...")
        workers = ["think_worker", "learning_worker", "analysis_worker", "dream_worker", "recall_worker"]
        
        # Phase 1: Think worker identifies a risk
        print("\n2. Think worker identifies SQL injection risk...")
        resolver = ConflictResolver()
        
        kw_think = KnowledgeExchange(
            id="kw_think_1",
            knowledge_type="risk_pattern",
            knowledge_content=json.dumps({
                "pattern": "sql_injection",
        ...
        print("\n3. Learning worker broadcasts similar risk knowledge...")
        kw_learning = KnowledgeExchange(
            id="kw_learning_1",
            knowledge_type="risk_pattern",
            knowledge_content=json.dumps({
                "pattern": "sql_injection",
        ...
        # This should be detected as duplicate/similar to think_worker's knowledge
        conflict = resolver.analyze_conflicts(kw_think, kw_learning)
        assert conflict.conflict_type == ConflictType.DUPLICATE
        assert conflict.similarity_score > 0.8
        print("✓ Cross-worker duplicate detection working")
        
        # Phase 3: Analysis worker provides additional context
        print("\n4. Analysis worker provides complementary knowledge...")
        kw_analysis = KnowledgeExchange(
            id="kw_analysis_1",
            knowledge_type="learned_pattern",
            knowledge_content=json.dumps({
        ...
        print("\n5. Dream worker proposes successful fix...")
        kw_dream = KnowledgeExchange(
            id="kw_dream_1",
        ...
        print("\n6. Recall worker provides historical pattern...")
        kw_recall = KnowledgeExchange(
            id="kw_recall_1",
        ...
        print("\n7. Testing conflict resolution in real scenario...")
...
async def test_performance_with_many_items():
...
def test_conflict_statistics():
    ...
    for i in range(3):
        kw1 = KnowledgeExchange(
            id=f"dup_stat_1_{i}",
            knowledge_type="learned_pattern",
            knowledge_content=json.dumps({"pattern": f"dup_{i}", "rate": 0.8}),
        ...
        kw2 = KnowledgeExchange(
            id=f"dup_stat_2_{i}",
            knowledge_type="learned_pattern",
            knowledge_content=json.dumps({"pattern": f"dup_{i}", "rate": 0.82}),
            topics=json.dumps(["test"]),
            source_worker="learning_worker",
            target_worker="analysis_worker",
            urgency="normal",
    ...
    for i in range(2):
        kw1 = KnowledgeExchange(
            ...
            knowledge_type="decision_outcome",
            knowledge_content=json.dumps({"decision": f"decision_a_{i}"}),
            topics=json.dumps(["test"]),
            source_worker="think_worker",
            target_worker="analysis_worker",
            urgency="normal",
        ...
        kw2 = KnowledgeExchange(
            ...
            knowledge_type="decision_outcome",
            knowledge_content=json.dumps({"decision": f"decision_b_{i}"}),
            topics=json.dumps(["test"]),
            source_worker="learning_worker",
            target_worker="analysis_worker",
            urgency="normal",
...
async def main():
    ...
    try:
        ...
        await test_performance_with_many_items()
